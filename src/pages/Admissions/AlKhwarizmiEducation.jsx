...
import school from "./images/Prezident1.jpg";
import AOS from "aos";
...
const FeatureCard = ({ icon, title, desc, color }) => (
  <motion.div
    className={`group relative flex flex-col gap-3 rounded-2xl bg-gradient-to-br ${color} p-6 shadow-md hover:shadow-xl transition-all border border-white/40 backdrop-blur-sm cursor-pointer`}
  >
    <div className="flex items-center gap-3">
      <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-white shadow-md ring-1 ring-slate-200 transition-transform duration-300 ease-out group-hover:translate-y-[-3px]">
        {icon}
      </div>
    ...
    <div className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-white/20 to-transparent opacity-0 group-hover:opacity-30 transition-opacity duration-300 ease-out" />
  </motion.div>
);
...
export default function AlKhwarizmiEducation() {
  ...
  return (
    <section
      className="relative font-[Poppins] w-full bg-white py-20 px-6 lg:px-12 overflow-hidden"
      ...
      >
        ...
        >
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 leading-tight">
            Muhammad al-Xorazmiy maktabida ta'lim
          </h2>
          <p className="text-lg text-slate-600 leading-relaxed mt-4 max-w-xl">
            Al-Xorazmiy maktabi  5–11-sinf o'quvchilariga chuqurlashtirilgan
            ta’lim beradi. Yo'nalishlar informatika,matematika, fizika va ingliz tili fanlarini qamrab oladi. 
          </p>
