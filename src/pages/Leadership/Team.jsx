...
const Team = () => {
  ...
  return (
    <section className="bg-gray-50 py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-16 font-poppins">
...
const MainCard = ({ member }) => (
  <div className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 text-center h-[450px] sm:h-[500px] md:h-[600px] flex flex-col">
    ...
    <div className="p-4 sm:p-5 md:p-6 flex flex-col justify-start">
      <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-900">{member.name}</h3>
      <div className="w-16 sm:w-20 h-[3px] bg-blue-500 mt-1 sm:mt-2 mb-2 sm:mb-3 mx-auto"></div>
      <p className="text-gray-600 text-sm sm:text-base leading-relaxed">{member.title}</p>
    </div>
  </div>
);
...
const SideCard = ({ member, side }) => (
  ...
  >
    ...
    <div className="w-1/2 p-3 sm:p-4 md:p-5">
      <h3 className="text-base sm:text-lg md:text-lg font-semibold text-gray-900">{member.name}</h3>
      <div
        className={`w-12 sm:w-16 h-[3px] bg-blue-500 mt-1 sm:mt-2 mb-1 sm:mb-2 md:mb-3 ${
          side === "right" ? "ml-auto" : ""
        }`}
      ></div>
      <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">{member.title}</p>
    </div>
  </div>
);
