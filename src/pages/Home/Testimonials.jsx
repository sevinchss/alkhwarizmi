...
export default function Testimonials() {
  ...
  const next = () => setCurrent((prev) => (prev + 2) % testimonials.length);
  const prev = () =>
    setCurrent((prev) =>
      prev - 2 < 0 ? testimonials.length - (testimonials.length % 2 || 2) : prev - 2
    );
    ...
    <section className="relative py-24  overflow-hidden font-poppins">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <button
            onClick={prev}
            className="absolute left-0 md:-left-12 bg-white/80 hover:bg-blue-100 text-blue-700 rounded-full p-3 shadow-md transition z-20"
          >
            <ChevronLeft size={28} />
          </button>

          {/* CARDS */}
          <div className="flex flex-col md:flex-row justify-center gap-8 w-full md:w-[85%] transition-all duration-700">
            {visible.map((item, index) => (
              <div
                key={index}
                className="relative bg-white rounded-3xl shadow-lg hover:shadow-2xl p-10 flex flex-col items-center justify-between text-center transition-all duration-500 border border-gray-100 hover:-translate-y-1 min-h-[420px] flex-1"
              >
                ...
                <img
                  src={item.image}
                  alt={item.name}
                  className="w-24 h-24 rounded-full border-4 border-blue-600 mb-6 object-cover"
                />
                <p className="text-gray-700 text-base md:text-lg leading-relaxed italic mb-6 flex-1">
                  “{item.text}”
                </p>
                <div>
                  ...
                  <p className="text-blue-600 text-sm mt-1">{item.role}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={next}
            className="absolute right-0 md:-right-12 bg-white/80 hover:bg-blue-100 text-blue-700 rounded-full p-3 shadow-md transition z-20"
          >
